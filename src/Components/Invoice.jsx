...
const statusList = ["All", "Draft", "Partially Paid", "Paid", "Due", "Overdue"];

const Invoice = ({ onNewClick, onClose }) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      setSelectedItems(invoiceTable.map((item) => item.id));
      setSelectAll(true);
    } else {
      setSelectedItems([]);
      setSelectAll(false);
          ...
          <Table stickyHeader sx={{ minWidth: 1200 }}>
            <TableHead>
              <TableRow>
                ...
                {["Date", "Invoice#", "Order#", "Customer Name", "Status", "Due Date", "Amount", "Balance Due"].map(h => (
