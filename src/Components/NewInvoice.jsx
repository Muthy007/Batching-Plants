...
const NewInvoice = ({ handleClose }) => {
  const [rows, setRows] = useState([{ id: 1, item: "", quantity: "1.00", rate: "0", amount: "0.00" }]);
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", backgroundColor: "#fff" }}>
      <Box sx={{ 
        ...
        <Typography sx={{ fontSize: { xs: "1.2rem", md: "1.5rem" }, fontWeight: "bold", color: "#333" }}>New Invoice</Typography>
      ...
      <Box sx={{ flex: 1, overflowY: "auto", p: { xs: "20px", md: "30px 50px" } }}>
        <Box sx={{ maxWidth: "800px" }}>
          ...
          <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "flex-start", sm: "center" }, mb: "14px", gap: { xs: 1, sm: 0 } }}>
            <Typography sx={{ ...labelStyle, color: "red" }}>Invoice Number*</Typography>
            <TextField value="INV-000001" sx={{ ...inputSx, width: { xs: "100%", sm: "240px" } }} InputProps={{ endAdornment: <InputAdornment position="end"><SettingsIcon sx={{ fontSize: 16, color: "#999" }} /></InputAdornment> }} />
          </Box>
          <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "flex-start", sm: "center" }, mb: "14px", gap: { xs: 1, sm: 0 } }}>
            <Typography sx={labelStyle}>Order Number</Typography>
            <TextField placeholder="Enter Order Number" sx={{ ...inputSx, width: { xs: "100%", sm: "400px" } }} />
          </Box>
          <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "flex-start", sm: "center" }, mb: "14px", gap: { xs: 1, sm: 0 } }}>
            <Typography sx={{ ...labelStyle, color: "red" }}>Invoice Date*</Typography>
            <Box sx={{ display: "flex", alignItems: "center", flexGrow: 1, gap: 1 }}>
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker defaultValue={dayjs()} slotProps={{ textField: { sx: { ...inputSx, width: "100%" } } }} />
                </LocalizationProvider>
                <Typography sx={{ fontSize: "13px", color: "#666", whiteSpace: "nowrap" }}>Due Date</Typography>
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker defaultValue={dayjs()} slotProps={{ textField: { sx: { ...inputSx, width: "100%" } } }} />
                </LocalizationProvider>
            </Box>
          ...
          <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "flex-start", sm: "center" }, mb: "14px", gap: { xs: 1, sm: 0 } }}>
            <Typography sx={labelStyle}>Terms</Typography>
            <Autocomplete options={["Due on Receipt", "Net 15", "Net 30"]} sx={{ width: "100%", maxWidth: "300px" }} renderInput={(params) => <TextField {...params} placeholder="Select Term" sx={inputSx} />} />
          </Box>
        </Box>
            ...
            <Table sx={{ tableLayout: "fixed" }}>
              <TableHead sx={{ backgroundColor: "#F9F9FB" }}>
                <TableRow>
                  ...
                  <TableCell sx={{ width: "15%", fontSize: "11px", fontWeight: 700 }}>QUANTITY</TableCell>
                  <TableCell sx={{ width: "15%", fontSize: "11px", fontWeight: 700 }}>RATE</TableCell>
                  <TableCell sx={{ width: "15%", fontSize: "11px", fontWeight: 700, textAlign: "right" }}>AMOUNT</TableCell>
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell><TextField defaultValue="0" sx={inputSx} /></TableCell>
                    <TableCell sx={{ textAlign: "right", fontSize: "13px" }}>0.00</TableCell>
