...
const SalesOrder = ({ onNewClick, onClose }) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      setSelectedItems(orderTable.map((item) => item.id));
      setSelectAll(true);
    } else {
      setSelectedItems([]);
      setSelectAll(false);
  ...
  const handleCheckboxClick = (event, id) => {
    ...
    setSelectAll(newSelected.length === orderTable.length);
  };
