...
const NewCreditNote = ({ handleClose }) => {
  const [items, setItems] = useState([{ id: Date.now(), item: null, quantity: 1, rate: 0, amount: 0 }]);
  const [creditNoteDate, setCreditNoteDate] = useState(dayjs());
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box sx={{ height: "100%", display: "flex", flexDirection: "column", bgcolor: "#fff" }}>
        <Box sx={{ 
          ...
          <Typography sx={{ fontSize: { xs: "1.2rem", md: "1.5rem" }, fontWeight: "bold", color: "#333" }}>New Credit Note</Typography>
        ...
        <Box sx={{ flexGrow: 1, overflowY: "auto", p: 4 }}>
          <Grid container spacing={2} sx={{ maxWidth: 800 }}>
             <Grid item xs={12} sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "flex-start", sm: "center" }, gap: { xs: 1, sm: 0 } }}>
                ...
                <Autocomplete size="small" fullWidth options={[]} renderInput={(params) => <TextField {...params} placeholder="Select Customer" />} />
             </Grid>

             <Grid item xs={12} sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "flex-start", sm: "center" }, gap: { xs: 1, sm: 0 } }}>
                <Typography sx={{ width: { xs: "100%", sm: 160 }, fontWeight: "600", color: "red", fontSize: "0.9rem" }}>Credit Note#*</Typography>
                <TextField size="small" fullWidth={false} sx={{ width: { xs: "100%", sm: "auto" } }} value={creditNoteNumber} onChange={(e) => setCreditNoteNumber(e.target.value)} />
             </Grid>

             <Grid item xs={12} sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "flex-start", sm: "center" }, gap: { xs: 1, sm: 0 } }}>
                <Typography sx={{ width: { xs: "100%", sm: 160 }, fontWeight: "600", color: "red", fontSize: "0.9rem" }}>Credit Note Date*</Typography>
                <DatePicker value={creditNoteDate} onChange={setCreditNoteDate} slotProps={{ textField: { size: 'small', fullWidth: true, sx: { width: { xs: "100%", sm: "auto" } } } }} />
             </Grid>
          </Grid>
