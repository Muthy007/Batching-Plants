...
import BotContent from "../../UsableContent/BotContent";
...
const NewQuotes = ({ onLogout }) => {
  ...
  const handleSetAmount = (index) => {
    ...
    if (selectedItemGST[index] && selectedItemGST[index].value) {
      ...
      gstAmount = (gstPercentage / 100) * calculatedAmount;
      calculatedAmount += gstAmount;
    }

    console.log("GST Amount Added:", gstAmount); // Logs the added GST amount
  ...
  const handleSaveQuote = (send, setLoading) => {
    setLoading(true);
    const itemsId =
    ...
    const params = {
      json_type: "save_quotes",
      cust_id: selectedCustomer?.id,
      plc_supply: stateId,
      quote_no: prefix,
      reference: reference,
      quote_date: dayjs(from).format("YYYY-MM-DD"),
      exp_date: dayjs(to).format("YYYY-MM-DD"),
    ...
    const fieldMessages = {
      ...
      quote_no: "Quote number is missing.",
      reference: "Reference is required.",
      quote_date: "Quote date is missing or invalid.",
      exp_date: "Expiration date is missing or invalid.",
  ...
  const handleSaveDraft = () => handleSaveQuote(false, setIsSavingDraft);
  const handleSend = () => handleSaveQuote(true, setIsSavingAndSending);
          ...
          <React.Fragment>
            ...
            >
              ...
              >
                ...
                >
                  <Grid>
                    <Tooltip title="Close Quotes" arrow>
                      <CloseIcon
                        onClick={() => {
                          navigate(-1);
                        }}
                      ...
                      <Grid item xs={12} md={3.7}>
                        <Autocomplete
                          ...
                          onChange={(e, val) => {
                            ...
                            setSelectedRate(val ? val.rate : null);
                          }}
                          getOptionLabel={(option) => option.display_name}
                          sx={{
                            "& + .MuiAutocomplete-popper .MuiAutocomplete-option:hover":
