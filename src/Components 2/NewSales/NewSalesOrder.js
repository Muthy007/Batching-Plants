...
const NewSalesOrder = ({ onLogout }) => {
  ...
  const handleSaveSalesOrder = (send, setLoading) => {
    setLoading(true);
    const itemsId =
    ...
    const params = {
      json_type: "save_sales_order",
      cust_id: selectedCustomer?.id,
      plc_supply: stateId,
      sales_order_no: prefix,
      reference: reference || "",
      sales_order_date: dayjs(from).format("YYYY-MM-DD"),
      shipment_date: dayjs(to).format("YYYY-MM-DD"),
      ...
      series_no: seriesNo,
      quote_no: orderNo || "",
      quote_ids: convert ? [ids] : quoteId ? [quoteId] : [],
      org_id: orgid,
      user_id: userid,
      tax_id: taxId || "0",
    ...
    const fieldMessages = {
      ...
      shipment_date: "Shipment date is missing or invalid.",
      terms: "Terms are not selected.",
      sales_person: "Salesperson is missing.",
  ...
  const handleSaveDraft = () => handleSaveSalesOrder(false, setIsSavingDraft);
  const handleSend = () => handleSaveSalesOrder(true, setIsSavingAndSending);
