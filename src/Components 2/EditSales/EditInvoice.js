...
const EditInvoice = ({ onLogout }) => {
  ...
  const getInvoiceDetails = () => {
    const params = {
      ...
      type: "2",
      invoice_ids: [invoiceDetails[0]?.id],
      user: user,
      org_id: orgid,
    ...
    axios
      ...
      .then((res) => {
        ...
        if (JSON.parse(res.data).json_sts === "1") {
          ...
          setPrefix(JsonData[0].invoice_no);
          setFrom(JsonData[0].invoice_date);
          setOrderNo(JsonData[0].order_numbers);
  ...
  const handleEditSaveInvoice = (send, setLoading) => {
    setLoading(true);
    const itemsId =
  ...
  const handleSaveDraft = () => handleEditSaveInvoice(false, setIsSavingDraft);
  const handleSend = () => handleEditSaveInvoice(true, setIsSavingAndSending);
