...
const EditCreditNote = ({ onLogout }) => {
  ...
  const { CreditDetails } = location.state || {};
  ...
  const getCreditDetails = () => {
    const params = {
      ...
      id: CreditDetails[0] ? CreditDetails[0].credit_note_id : "",
      org_id: orgid,
      type: "2",
        ...
        if (JSON.parse(res.data).json_sts === "1") {
          ...
          if (customerDetails) {
            const selectedCustomer = {
              ...
              display_name: customerDetails[0].customer_name,
            };
            setSelectedCustomer(selectedCustomer);
            handleCustomerSelect(selectedCustomer ? selectedCustomer.id : null); // If needed
            getInvoiceDetails(selectedCustomer.id);
            console.log(selectedCustomer);
          }

          setSubject(JsonData[0].subject);
          ...
          setPrefix(JsonData[0].credit_no);
          setFrom(JsonData[0].credit_date);
          handleMenuItemClick(
  ...
  useEffect(() => {
    const InvoiceTypeOption =
      invoiceTypeDetails &&
      invoiceTypeDetails.find(
        (option) => option.name === CreditDetails[0]?.invoice_type
      );
    if (InvoiceTypeOption) {
      setSelectedInvoiceType(InvoiceTypeOption);
    }

    const reasonOption =
      Reasons && Reasons.find((option) => option === CreditDetails[0]?.reason);
    if (reasonOption) {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (JSON.parse(res.data).json_sts === "1") {
          ...
          const InvoiceOption = JsonData.find(
            (option) => option.invoice_no === CreditDetails[0]?.invoice_no
          );
          if (InvoiceOption) {
  ...
  const handleEditSaveCreditNotes = (stage, setLoading) => {
    setLoading(true);
    // const accounts =
    ...
    const params = {
      json_type: "edit_credit_note",
      cust_id: selectedCustomer?.id,
      plc_supply: stateId,
      id: CreditDetails[0]?.credit_note_id || "",
  ...
  const handleSaveDraft = () =>
    handleEditSaveCreditNotes("1", setIsSavingDraft);
  const handleSaveAsOpen = () =>
    handleEditSaveCreditNotes("2", setIsSavingAndSending);
