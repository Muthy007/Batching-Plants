...
const EditDeliveryChallan = ({ onLogout }) => {
  ...
  const getDeliveryChallan = () => {
    const params = {
      json_type: "get_delivery_chellan",
      type: "2",
      delivery_ids: [deliveryDetails[0]?.id],
      user: user,
      org_id: orgid,
    ...
    axios
      ...
      .then((res) => {
        ...
        if (JSON.parse(res.data).json_sts === "1") {
          ...
          setPrefix(JsonData[0].delivery_no);
          setReference(JsonData[0].reference);
          setFrom(JsonData[0].delivery_date);
          setSelectedChallanType(JsonData[0].challan_type);
  ...
  const handleEditSaveDeliveryChallan = () => {
    setLoading(true);
    const itemsId =
    ...
    const params = {
      json_type: "edit_delivery_challan",
      cust_id: selectedCustomer?.id,
      bill_id:
      ...
      user_id: userid,
      user: user,
      delivery_challan_id: deliveryDetails[0]?.id,
    };
    ...
    const fieldMessages = {
      ...
      delivery_challan_id: "Delivery Challan ID is missing.",
    };
    ...
    axios
      ...
      .catch((error) => {
        console.error("Error editing delivery challan:", error);
