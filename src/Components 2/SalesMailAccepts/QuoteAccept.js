...
const QuoteAccept = ({ onLogout }) => {
  ...
  const handleAcceptOrDecline = (acceptordecline) => {
    const params = {
      json_type: "respond_to_document",
      type: "quote",
      response: acceptordecline === "accept" ? "accept" : "decline",
      org_id: orgid,
      user: user,
      comment: commentCheck === "ok" ? comment : "",
      id: item.id,
    };
    ...
    axios
      ...
      .then((res) => {
        ...
        if (JsonData.json_sts === "1") {
          ...
          setSnackbarMessage("Failed to process the request.");
                ...
                >
                  <Toolbar>
                    <IconButton
                      ...
                      onClick={() => {
                        navigate(
                          `/Email/Sales/Quote?organisationId=${organisationId}&organisation=${organisation}&customerId=${customerId}`
                        );
                      }}
                    >
                      <ArrowBackIcon />
                    ...
                    <Grid container justifyContent="flex-end" spacing={2}>
                      {quoteDetails[0]?.status === "Accepted" ? (
                        ""
                      ) : (
                        <Grid item>
                          <Button
                            variant="contained"
                            sx={{ backgroundColor: "#408DFB" }}
                            onClick={() => {
                              handleAcceptOrDecline("accept");
                            }}
                          >
                            Accept
                          </Button>
                      ...
                      {quoteDetails[0]?.status === "Declined" ? (
                        ""
                      ) : (
                        <Grid item>
                          <Button
