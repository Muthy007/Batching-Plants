...
const status = ["All", "Draft", "Open"];

const CreditNotes = ({ onLogout }) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const selected = creditTable.map((item) => item.credit_note_id); // Assuming each item has a unique identifier like `id`
  ...
  const handleCheckboxClick = (event, id) => {
    ...
    setSelectAll(newSelected.length === creditTable.length);
  };
  ...
  const handleDelete = () => {
    ...
    const filteredTable = creditTable
      .filter((item) => selectedIds.includes(item.credit_note_id)) // Adjust this condition as per your data structure
      .map((item) => item.credit_note_id); // Store only the id as an array
    const params = {
      json_type: "delete_credit_note",
      credit_note_ids: filteredTable,
      user: user,
      org_id: orgid,
      user_id: userid,
    };
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {selectedItem && creditTable.length === 0 && (
                    <Box
                      sx={{
                        textAlign: "center",
                        p: 20,
                    ...
                    >
                      ...
                      >
                        Currently, no Credit Notes are available. Select 'Create
                        New' to add a new one.
                      </Typography>
                      <Tooltip title="Create Creditnotes" arrow>
                        <Button
                          sx={{
                            width: "200px",
                            height: 50,
                  ...
                  {creditTable.length > 0 &&
                    ...
                    !creditTable
                      ...
                      >
                        ...
                        >
                          There are no {selectedItem} Credit Notes
                        </Typography>
                      </Box>
                    )}
                  {creditTable.length > 0 &&
                    !creditTable.filter((data) =>
                      Object.values(data).some((row) =>
                        row
                          .toString()
                          .toLowerCase()
